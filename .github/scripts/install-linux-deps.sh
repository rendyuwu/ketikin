...
# after it — observed twice in a row on ketikin#17, 21m43s of silence on one
# runner and 1h02m on the next, ten minutes after the same job passed in four.
# Falling back works for InRelease and then does not work for Packages, so this
# is not something a retry of the whole step fixes.
#
# Two defences, because either alone is a guess about which layer breaks:
#   1. Repoint every apt source away from the Azure mirror onto archive.ubuntu.com,
#      which is what apt had already failed over to on its own.
